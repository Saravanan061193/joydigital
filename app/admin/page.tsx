...
export default function AdminPage() {
  ...
  // Live Chat Metrics Calculations
  const totalChatSessions = chatSessions.length;
  const todayChatSessions = chatSessions.filter((s) => {
  ...
  const leadsFromChat = chatSessions.filter((s) => Boolean(s.leadId)).length;
  const totalChatMessages = chatSessions.reduce((acc, s) => acc + (s.messageCount || 0), 0);
  const chatConversionRate = totalChatSessions > 0 ? Math.round((leadsFromChat / totalChatSessions) * 100) : 0;
    ...
    <div className={`min-h-screen flex text-slate-800 relative overflow-hidden inter-font select-none ${isDarkMode ? "dark dark-theme bg-[#0B0F19] text-slate-200" : "bg-[#F8FAFC] text-slate-800"}`}>
      ...
      <main className="flex-1 h-screen overflow-y-auto bg-[#F8FAFC] dark:bg-[#0B0F19] flex flex-col min-w-0 transition-colors duration-200">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-8 max-w-[1440px] w-full mx-auto">
          ...
          {activeTab === "chats" && (
            <div className="max-w-7xl mx-auto mb-12 animate-fade-in text-left">
              
              {/* TOP METRICS SUMMARY BAR FOR LIVE CHAT LOGS */}
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6 select-none">
                {/* 1. Total Conversations */}
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-4 shadow-[0_4px_20px_rgba(15,23,42,0.03)] flex items-center justify-between">
                  <div className="space-y-0.5">
                    <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Total Conversations</span>
                    <span className="text-xl font-black text-slate-900 dark:text-white leading-none block">{totalChatSessions} Chats</span>
                    <span className="text-[10px] text-[#2563EB] dark:text-blue-400 font-bold block">All-time logged</span>
                  </div>
                  <div className="w-10 h-10 rounded-xl bg-blue-50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/30 flex items-center justify-center text-[#2563EB] dark:text-blue-400 text-sm shrink-0">
                    <i className="fa-regular fa-comments" />
                  </div>
                </div>

                {/* 2. Today's Active Chats */}
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-4 shadow-[0_4px_20px_rgba(15,23,42,0.03)] flex items-center justify-between">
                  <div className="space-y-0.5">
                    <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Today's Chats</span>
                    <span className="text-xl font-black text-emerald-600 dark:text-emerald-400 leading-none block">{todayChatSessions} Active</span>
                    <span className="text-[10px] text-emerald-600 font-bold flex items-center gap-1">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" /> Live today
                    </span>
                  </div>
                  <div className="w-10 h-10 rounded-xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30 flex items-center justify-center text-emerald-600 dark:text-emerald-400 text-sm shrink-0">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-4 shadow-[0_4px_20px_rgba(15,23,42,0.03)] flex items-center justify-between">
                  <div className="space-y-0.5">
                    <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Leads Generated</span>
                    <span className="text-xl font-black text-purple-600 dark:text-purple-400 leading-none block">{leadsFromChat} Leads</span>
                    <span className="text-[10px] text-purple-600 dark:text-purple-400 font-bold block">{chatConversionRate}% Chat Conversion</span>
                  </div>
                  <div className="w-10 h-10 rounded-xl bg-purple-50 dark:bg-purple-950/20 border border-purple-100 dark:border-purple-900/30 flex items-center justify-center text-purple-600 dark:text-purple-400 text-sm shrink-0">
                    <i className="fa-solid fa-user-check" />
                  </div>
                </div>

                {/* 4. Total Messages Exchanged */}
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-4 shadow-[0_4px_20px_rgba(15,23,42,0.03)] flex items-center justify-between">
                  <div className="space-y-0.5">
                    <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider block">Total Messages</span>
                    <span className="text-xl font-black text-amber-600 dark:text-amber-400 leading-none block">{totalChatMessages.toLocaleString()} Msgs</span>
                    <span className="text-[10px] text-amber-600 font-bold block">Avg {totalChatSessions > 0 ? Math.round(totalChatMessages / totalChatSessions) : 0} msgs/chat</span>
                  </div>
                  <div className="w-10 h-10 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/30 flex items-center justify-center text-amber-600 dark:text-amber-400 text-sm shrink-0">
              ...
              {/* CHAT SESSION LIST & TRANSCRIPT DISPLAY GRID */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-16rem)] min-h-[500px]">
